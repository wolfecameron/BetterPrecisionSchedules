...
@dataclass
class QTrainingArguments(TrainingArguments):
    """extends the training arguments class to contain quantization parameters"""

    # cpt arguments
    precision_schedule: str = field(
        default='fixed',
        metadata={
            "help": "the type of schedule to be used for cyclic precision training",
        },
    )
    num_bits: int = field(
        default=8,
        metadata={
            "help": "bits in current forward pass",
        },
    )
    num_grad_bits: int = field(
        default=8,
        metadata={
            "help": "bits in current backward pass",
        },
    )
    num_bits_min: int = field(
        default=8,
        metadata={
            "help": "min forward pass bits",
        },
    )
    num_bits_max: int = field(
        default=8,
        metadata={
            "help": "max forward pass bits",
        },
    )
    num_grad_bits_min: int = field(
        default=8,
        metadata={
            "help": "min backward pass bits",
        },
    )
    num_grad_bits_max: int = field(
        default=8,
        metadata={
            "help": "max backward pass bits",
        },
    )
    num_cyclic_period: int = field(
        default=8,
        metadata={
            "help": "number of precision schedules to perform throughout training",
        },
    )
    flip_vertically: bool = field(
        default=False,
        metadata={
...
def main():
    ...
    parser = HfArgumentParser((ModelArguments, DataTrainingArguments, QTrainingArguments))
    ...
    # create quantized model with weights from pre-trained model
    model = QBertForSequenceClassification(tmp_model.config, training_args)
    safe_load_dict(model, tmp_model.state_dict()) # all quantization modules will fail to load
