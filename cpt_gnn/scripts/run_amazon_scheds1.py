...
c = 8
trials = 2
save_name = './results/amazon_sage_qagg_sched_00.json'
tmp_fn = 'sage_amazon_res1.txt'
...
for nb, ngb in zip(nbs, ngbs):
    for ps in qscheds:
        if ps in ['demon_decay', 'exp_decay']:
            ...
            exp_name = f'amazon_qagg_{ps}_{nb[0]}_{nb[-1]}_vertical'
            ...
            exp_name = f'amazon_qagg_{ps}_{nb[0]}_{nb[-1]}_horizontal'
            for t in range(trials):
                full_exp_name = exp_name + f'_{t}'
                command = (
                    f'CUDA_VISIBLE_DEVICES={gpu} python train_amazon.py --exp-name {full_exp_name} '
            ...
            # run vertical flip trials
            exp_name = f'amazon_qagg_{ps}_{nb[0]}_{nb[-1]}'
            for t in range(trials):
                full_exp_name = exp_name + f'_{t}'
                command = (
                    f'CUDA_VISIBLE_DEVICES={gpu} python train_amazon.py --exp-name {full_exp_name} '
