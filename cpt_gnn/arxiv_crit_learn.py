...
def main():
    ...
    # cpt_params
    parser.add_argument('--def-epochs', default=100, type=int)
...
def adjust_learning_rate(args, optimizer, _epoch):
    ...
    elif args.lr_schedule == 'cosine-no-def-decay':
        if _epoch < args.def_epochs:
            lr = args.lr
        else:
            non_def_epochs = int(args.n_epochs - args.def_epochs)
            decay_epoch = int(_epoch - args.def_epochs)
            lr_min = args.lr * 0.1
            lr_max = args.lr
            lr = lr_min + 1 / 2 * (lr_max - lr_min) * (1 + np.cos(decay_epoch / non_def_epochs * 3.141592653))
