...
def calc_demon_decay(cyclic_period, curr_iter, min_val, max_val, discrete=True):
    assert min_val <= max_val
...
def calc_exp_decay(cyclic_period, curr_iter, min_val, max_val, exponent=4, discrete=True):
    assert min_val <= max_val
    z = float(curr_iter % cyclic_period) / cyclic_period
...
if __name__=='__main__':
    ...
    dd = [calc_demon_decay(total_iter, i, 3.0, 8.0, discrete=discrete) for i in range(total_iter)]
    dg = [calc_demon_growth(total_iter, i, 3.0, 8.0, discrete=discrete) for i in range(total_iter)]
    ed = [calc_exp_decay(total_iter, i, 3.0, 8.0, discrete=discrete) for i in range(total_iter)]
