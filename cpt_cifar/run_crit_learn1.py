...
for it, dit in zip(iters, def_iters):
    for t in range(trials):
        exp_name = f'critlearn_{dataset}_{arch}_{lr_sched}_{dit}_quant_min{min_bit}_{t}/'
        ...
        command = (f'CUDA_VISIBLE_DEVICES={gpu} python crit_learn.py --cmd train --exp-name {exp_name} '
                ...
                f'--cyclic_num_bits_schedule {num_bits} --cyclic_num_grad_bits_schedule {num_grad_bit} --use-wandb ')
        if warm_up:
            command += ' --warm_up'
        if pretrained:
            command += ' --pretrained'
